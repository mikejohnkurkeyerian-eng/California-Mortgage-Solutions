...
function RegisterForm() {
    ...
    const initialRole = searchParams.get('role') as 'BORROWER' | 'BROKER' | null;

    const [role, setRole] = useState<'BORROWER' | 'BROKER'>(initialRole || 'BORROWER');
    ...
    // Effect to enforce role if provided in URL
    useEffect(() => {
        if (initialRole && (initialRole === 'BORROWER' || initialRole === 'BROKER')) {
            setRole(initialRole);
        }
    }, [initialRole]);

    const getTitle = () => {
        if (initialRole === 'BROKER') return 'Join Broker Network';
        if (initialRole === 'BORROWER') return 'Create Borrower Account';
