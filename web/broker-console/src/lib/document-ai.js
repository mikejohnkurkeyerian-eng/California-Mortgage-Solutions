...
import { BrokerIntelligenceService } from './broker-intelligence';
...
function performMultiPageOCR(file_1) {
    return __awaiter(this, arguments, void 0, function* (file, maxPages = 5) {
        const arrayBuffer = yield file.arrayBuffer();
        const pdf = yield pdfjsLib.getDocument({ data: arrayBuffer }).promise;
        let fullText = "";
        ...
        for (let i = 1; i <= pagesToScan; i++) {
            ...
            if (blob) {
                const { data } = yield Tesseract.recognize(blob, 'eng+spa', // Support English and Spanish
        ...
        try {
            ...
            // 2. PDF Handling (Hybrid Approach)
                ...
                try {
                    ...
                    text = yield extractTextFromPdf(file);
                    ...
                    if (text.trim().length < 50) {
                        ...
                        text = yield performMultiPageOCR(file);
                    }
                }
                catch (pdfError) {
            ...
            // 3. Image Handling (Single Page OCR)
                ...
                const { data } = yield Tesseract.recognize(file, 'eng+spa', { logger: (m) => console.log(m) });
                text = data.text;
            }
            ...
            if (text.trim().length < 10) {
                ...
                if (name.includes('w2') || name.includes('w-2'))
                    detectedType = 'W2';
                else if (name.includes('pay') || name.includes('stub'))
                    detectedType = 'PAY_STUB';
                else if (name.includes('bank') || name.includes('stmt'))
                    detectedType = 'BANK_STATEMENT';
                else if (name.includes('id') || name.includes('license'))
                    detectedType = 'ID';
                else if (name.includes('1040') || name.includes('tax'))
                    detectedType = 'TAX_RETURN';
                else if (name.includes('1099'))
                    detectedType = 'FORM_1099';
                else if (name.includes('asset') || name.includes('401k') || name.includes('investment'))
                    detectedType = 'ASSET_STATEMENT';
                else if (name.includes('business') && name.includes('license'))
                    detectedType = 'BUSINESS_LICENSE';
                else if ((name.includes('profit') && name.includes('loss')) || name.includes('p&l'))
                    detectedType = 'PROFIT_AND_LOSS';
            }
