export class Form1003Parser {
    ...
    static parseBorrowerInfo(text, app) {
        // Name
        ...
        if (employerMatch)
            app.employment.employerName = employerMatch[1].trim();
    ...
    static parseAssetsAndLiabilities(text, app) {
    ...
    static parseLoanAndPropertyInfo(text, app) {
        // Loan Amount
        ...
        // Loan Purpose
        if (text.includes('loan purpose') && text.includes('purchase'))
            app.loanPurpose = 'Purchase';
        else if (text.includes('refinance'))
            app.loanPurpose = 'Refinance';
    }
    static parseLoanOriginatorInfo(text, app) {
        // Loan Officer Info
