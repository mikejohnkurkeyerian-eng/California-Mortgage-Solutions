...
function LoanApplicationContent() {
    ...
    useEffect(() => {
        ...
        useEffect(() => {
            setFormInsights(FormIntelligenceService.evaluateForm(formData));
            // eslint-disable-next-line react-hooks/exhaustive-deps
        }, [formData]); // Run whenever form data changes to keep indicators fresh
                            ...
                            <div className="flex justify-between items-center -mx-4 md:-mx-16 lg:-mx-24 mb-6 px-4 md:px-0 relative">
                                ...
                                {['Borrower', 'Employment', 'Assets', 'Liabilities', 'Real Estate', 'Prop/Loan', 'Declarations', 'Agreements', 'Military'].map((label, i) => {
                                    ...
                                    const hasErrors = isEditMode && formInsights.some(insight => {
                                        if (insight.severity !== 'ERROR' && insight.severity !== 'WARNING') return false;

                                        // Check if this insight matches the current step's fields
                                        return stepFields.some(prefix => {
                                            // 1. Direct field match
                                            if (insight.field.startsWith(prefix) || insight.field === prefix) return true;

                                            // 2. Action payload field match (many insights point to loanAndProperty but reference source field here)
                                            const payloadField = insight.action?.payload?.field;
                                            if (payloadField && (payloadField.startsWith(prefix) || payloadField === prefix)) return true;

                                            // 3. Target fields match
                                            const targetFields = insight.action?.payload?.targetFields;
                                            if (Array.isArray(targetFields) && targetFields.some(t => t.startsWith(prefix))) return true;
